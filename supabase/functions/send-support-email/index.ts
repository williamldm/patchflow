...
serve(async (req) => {
  ...
  try {
    ...
    const displayName = userName || user.user_metadata?.full_name || user.email;
...
<div style="max-width:560px;margin:0 auto;background:#1a2340;border-radius:12px;overflow:hidden;border:1px solid #2c3f5f">
  ...
  <div style="padding:24px">
    ...
    await Promise.all([
      ...
      sendEmail(SUPPORT_EMAIL, `[Support] ${subject} — ${displayName} (${user.email})`, teamHtml),
    ]);
